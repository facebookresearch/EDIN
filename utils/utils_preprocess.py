...
def split_paragraph_max_seq_length(text, f_out, tokenizer, seq_length=256):
    ...
    for sentence in sentences:
        ...
        if current_length + len(sentence_tokenized) <= seq_length:
            ...
            data = {"text": current_paragraph.strip()}
            if 'metadata' in data:
                data['data_example_id'] = text['metadata']['data_example_id'] + "_" + str(num)
                data["data_source_name"] = text['metadata']["data_source_name"]
                data["time_stamp"] = text['metadata']["time_stamp"]
            else:
                data['data_example_id'] = str(text['data_example_id']) + "_" + str(num)
                data["time_stamp"] = text["time_stamp"]
